...
import { ReactComponent as CaretRight } from 'assets/icons/caret-right-solid.svg'
...
const Sidebar: React.FC<Props> = ({
  ...
  const styles = useSpring({
    transform: `translateX(${show ? '0px' : '400px'})`
  })

  return (
    <>
      <S.Toggler onClick={() => setShow(!show)}>
        <CaretRight style={{ transform: `rotate(${ show ? 0 : 180}deg)` }} />
      </S.Toggler>
      <S.Sidebar style={styles}>
        <S.Player>
          <S.TrackInfo>
